...
def test_galatea(self):
    ...
    response = self.client.post(
        url_for('galatea.registration', lang=self.language),
        data=dict( name = self.email.split('@')[0],
        email = self.email,
    ...
    assert ('An email has been sent to activate your account'
    ...
    response = self.client.post(
        url_for('galatea.activate', lang=self.language),
        data=dict(
...
def test_reset_password(self):
    ...
    # reset password
    response = self.client.post(
        url_for('galatea.reset-password', lang=self.language),
